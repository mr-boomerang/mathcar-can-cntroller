 ...
 **/
#ifndef INTERFACES_PERIPHERALS_SIDEWINDER_H
#define INTERFACES_PERIPHERALS_SIDEWINDER_H
...
/// @brief Actions for Wheel Encoder joystick
class Sidewinder : public ControllerBase
{
 private:
  std::string last_gear = "neutral";
  double last_time = 0;
  double speed = 0;
  double steer = 0;
 ...
 public:
  ...
  /**
   * @brief to execute the Accelaration of the particular joystick
   * @param throttle value to set for throttle
   * @param brake value to set for brake
   */
  void ExecuteThrottle(double throttle, double brake);
  /**
  ...
  /**
   * @brief to get event number associated with the joystick
   */
  std::string GetEventNo();
  /**
   * @brief to set froce feedback value in the controller
   * @param value percentage stiffness
   */
  void SetForceFeedback(int value);
};

#endif  // INTERFACES_PERIPHERALS_SIDEWINDER_H
